...
exports.isArray =
  ...
  exports.accumulatingWindow =
  exports.mappedSlidingWindow =
...
function getCurrentItems(selectedItems, currentItem) {
  const { index } = currentItem.idx;
  var isAddedindex = selectedItems.findIndex(({ idx }) => idx.index === index);
  let newLinst = [...selectedItems];
  if (isAddedindex === -1) {
    newLinst = [...newLinst, currentItem];
  }
  return newLinst;
}
...
function find(list, predicate) {
  var context =
    arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : this;
...
function getClosestItem(array, value, accessor, log) {
  var _getClosestItemIndexe = getClosestItemIndexes(
      array,
      value,
      accessor,
      log,
...
var overlayColors = (exports.overlayColors = (0, _d3Scale.scaleOrdinal)(
  _d3Scale.schemeCategory10,
));
...
function isObject(d) {
  return (
    isDefined(d) &&
    (typeof d === 'undefined' ? 'undefined' : _typeof(d)) === 'object' &&
    !Array.isArray(d)
  );
}
