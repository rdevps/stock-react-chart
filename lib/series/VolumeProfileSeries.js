...
var VolumeProfileSeries = (function (_Component) {
  ...
  function VolumeProfileSeries(props) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (
        VolumeProfileSeries.__proto__ ||
        Object.getPrototypeOf(VolumeProfileSeries)
      ).call(this, props),
    );
...
function helper(props, moreProps, xAccessor, width) {
  ...
  var sessionBuilder = (0, _utils.accumulatingWindow)()
    .discardTillStart(!partialStartOK)
    .discardTillEnd(!partialEndOK)
    .accumulateTill(function (d, i) {
  ...
  var dx =
    plotData.length > 1
      ? realXScale(xAccessor(plotData[1])) -
        realXScale(xAccessor((0, _utils.head)(plotData)))
      : 0;
  ...
  var allRects = sessions.map(function (session) {
    var begin = bySession
      ? realXScale(xAccessor((0, _utils.head)(session)))
      : 0;
    var finish = bySession
      ? realXScale(xAccessor((0, _utils.last)(session)))
      : width;
    ...
    var rollup = (0, _d3Collection.nest)()
      .key(function (d) {
        return d.direction;
      })
      .sortKeys(orient === 'right' ? _d3Array.descending : _d3Array.ascending)
    ...
    var xScale = (0, _d3Scale.scaleLinear)()
      .domain([0, (0, _d3Array.max)(volumeValues)])
      .range([start, end]);
    ...
    var sessionBg = {
      ...
      height:
        (0, _utils.head)(rects).y -
        (0, _utils.last)(rects).y +
        (0, _utils.head)(rects).height,
      width: sessionWidth,
    };
  ...
  return {
    rects: (0, _d3Array.merge)(
      allRects.map(function (d) {
        return d.rects;
      }),
    ),
