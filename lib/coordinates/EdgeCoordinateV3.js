...
function renderSVG(props) {
  ...
  var from = props.from;
  var timerEnabled = props.timerEnabled;
  ...
  if ((0, _utils.isDefined)(edge.coordinateBase)) {
    ...
    if (timerEnabled && timer) {
      rectHeight = rectHeight + rectHeight / 2;
    }
    ...
    coordinate = _react2.default.createElement(
      ...
      {
        key: 2,
        x: edge.coordinate.edgeXText + 6 - rectWidth / 2,
        y:
          from === 'mouse'
            ? edge.coordinate.edgeYText + 1.5
            : edge.coordinate.edgeYText - 1.5,
        textAnchor: 'start',
      ...
      timerEnabled && timer
        ? _react2.default.createElement(
          'text',
          {
            key: 3,
            x: edge.coordinate.edgeXText + 6 - rectWidth / 2,
            y: edge.coordinate.edgeYText + 12.5,
            textAnchor: 'start',
            fontFamily: edge.coordinate.fontFamily,
            fontSize: edge.coordinate.fontSize,
            dy: '.32em',
