...
var EachFibRetracement = (function (_Component) {
  ...
  function EachFibRetracement(props) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (
        EachFibRetracement.__proto__ ||
        Object.getPrototypeOf(EachFibRetracement)
      ).call(this, props),
    );
    ...
    },
    {
      ...
      value: function render() {
        ...
        var _props12 = this.props,
          ...
          type = _props12.type,
          selectedInteraction = _props12.selectedInteraction,
        ...
        var dir =
          (0, _utils.head)(lines).y1 > (0, _utils.last)(lines).y1 ? 3 : -1.3;
        ...
        return _react2.default.createElement(
          ...
          lines.map(function (line, j) {
            var text =
              yDisplayFormat(line.y) + ' (' + line.percent.toFixed(2) + '%)';
            ...
            var interactiveCursorClass = firstOrLast
              ? 'react-stockcharts-ns-resize-cursor'
              : 'react-stockcharts-move-cursor';

            var interactiveEdgeCursorClass = firstOrLast
              ? 'react-stockcharts-ns-resize-cursor'
              : 'react-stockcharts-ew-resize-cursor';

            var dragHandler =
              j === 0
                ? _this2.handleLineNSResizeTop
                : j === lines.length - 1
                ? _this2.handleLineNSResizeBottom
                : _this2.handleLineMove;

            var edge1DragHandler =
              j === 0
                ? _this2.handleLineNSResizeTop
                : j === lines.length - 1
                ? _this2.handleLineNSResizeBottom
                : _this2.handleEdge1Drag;
            var edge2DragHandler =
              j === 0
                ? _this2.handleLineNSResizeTop
                : j === lines.length - 1
                ? _this2.handleLineNSResizeBottom
                : _this2.handleEdge2Drag;

            var hoverHandler =
              interactive && selectedInteraction
                ? { onHover: _this2.handleHover, onUnHover: _this2.handleHover }
                : {};
            return _react2.default.createElement(
              'g',
              { key: j },
