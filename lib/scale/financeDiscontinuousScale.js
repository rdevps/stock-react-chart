...
function financeDiscontinuousScale(index, futureProvider) {
  var backingLinearScale =
    arguments.length > 2 && arguments[2] !== undefined
      ? arguments[2]
      : (0, _d3Scale.scaleLinear)();
  ...
  scale.ticks = function (m, flexTicks) {
    ...
    var start =
      Math.max(Math.ceil(domainStart), (0, _utils.head)(index).index) +
      Math.abs((0, _utils.head)(index).index);
    var end =
      Math.min(Math.floor(domainEnd), (0, _utils.last)(index).index) +
      Math.abs((0, _utils.head)(index).index);

    if (Math.floor(domainEnd) > end) {
    }

    var desiredTickCount = Math.ceil(
      ((end - start) / (domainEnd - domainStart)) * backingTicks.length,
    );

    for (var i = MAX_LEVEL; i >= 0; i--) {
      var ticksAtLevel = ticksMap.get(i);
      var temp = (0, _utils.isNotDefined)(ticksAtLevel)
        ? []
        : ticksAtLevel.slice();
    ...
    for (var _i = MAX_LEVEL; _i >= 0; _i--) {
      if (
        ticksMap.get(_i).length + unsortedTicks.length >
        desiredTickCount * 1.5
      )
        break;
      unsortedTicks = unsortedTicks.concat(
        ticksMap.get(_i).map(function (d) {
          return d.index;
        }),
      );
    }
    ...
    if (!flexTicks && end - start > ticks.length) {
      ...
      var distance = Math.ceil(
        (backingTicks.length > 0
          ? ((0, _utils.last)(backingTicks) - (0, _utils.head)(backingTicks)) /
            backingTicks.length /
            4
          : 1) * 1.5,
      );
      ...
      for (var _i2 = 0; _i2 < ticks.length - 1; _i2++) {
        for (var _j = _i2 + 1; _j < ticks.length; _j++) {
          if (ticks[_j] - ticks[_i2] <= distance) {
            ticksSet.remove(
              index[ticks[_i2] + d].level >= index[ticks[_j] + d].level
                ? ticks[_j]
                : ticks[_i2],
            );
          }
