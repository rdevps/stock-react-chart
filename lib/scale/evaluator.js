...
exports.default = function (_ref2) {
  ...
  return extentsWrapper(
    useWholeData || (0, _utils.isNotDefined)(xScale.invert),
    clamp,
  ...
  function filterData(data, inputDomain, xAccessor, initialXScale) {
    ...
    if (typeof clamp === 'function') {
      clampedDomain = clamp(clampedDomain, [
        xAccessor((0, _utils.head)(data)),
        xAccessor((0, _utils.last)(data)),
      ]);
    } else {
      if (clamp === 'left' || clamp === 'both' || clamp === true) {
        clampedDomain = [
          Math.max(left, xAccessor((0, _utils.head)(data))),
          clampedDomain[1],
        ];
      }
      ...
      if (clamp === 'right' || clamp === 'both' || clamp === true) {
        clampedDomain = [
          clampedDomain[0],
          Math.min(right, xAccessor((0, _utils.last)(data))),
        ];
      }
    }
    ...
    var width = Math.floor(
      xScale(xAccessor((0, _utils.last)(filteredData))) -
        xScale(xAccessor((0, _utils.head)(filteredData))),
    );
    ...
    var chartWidth =
      (0, _utils.last)(xScale.range()) - (0, _utils.head)(xScale.range());
    ...
    log(
      'Trying to show ' +
        ...
        ('Also FYI the entire chart width is ' +
          chartWidth +
          'px and pointsPerPxThreshold is ' +
          pointsPerPxThreshold),
    );
    ...
    if (
      ...
      if (
        ...
        (0, _utils.isDefined)(fallbackEnd)
      ) {
        plotData = filteredData;
        var newEnd = getNewEnd(
          fallbackEnd,
          xAccessor,
          initialXScale,
          (0, _utils.head)(realInputDomain),
        );
        ...
        var newWidth = Math.floor(
          newXScale(xAccessor((0, _utils.last)(plotData))) -
            newXScale(xAccessor((0, _utils.head)(plotData))),
        );

        log(
          'and ouch, that is too much, so instead showing ' +
            plotData.length +
            ' in ' +
            newWidth +
            'px',
        ...
        domain = currentDomain || [
          xAccessor((0, _utils.head)(plotData)),
          xAccessor((0, _utils.last)(plotData)),
        ];

        var _newXScale = xScale.copy().domain(domain);
        var _newWidth = Math.floor(
          _newXScale(xAccessor((0, _utils.last)(plotData))) -
            _newXScale(xAccessor((0, _utils.head)(plotData))),
        );
...
function getFilteredResponse(data, left, right, xAccessor) {
  var newLeftIndex = (0, _utils.getClosestItemIndexes)(
    data,
    left,
    xAccessor,
  ).right;
  var newRightIndex = (0, _utils.getClosestItemIndexes)(
    data,
    right,
    xAccessor,
  ).left;
